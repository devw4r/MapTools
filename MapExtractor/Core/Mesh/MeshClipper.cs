...
namespace AlphaCoreExtractor.Core.Mesh
{
    public class MeshClipper : IDisposable
    {
        private const float epsilon = 1e-3f;
        private const float cellSize = 1e-3f;

        private Rect Bounds;
        private Dictionary<VertCoord, CVertex> CVertices = new Dictionary<VertCoord, CVertex>(50000);
        private List<CFace> CFaces = new List<CFace>(150000);
        ...
        public void Dispose()
        {
            foreach (var vtx in CVertices)
                StorageRoom.PushVector3(vtx.Value.Point);
